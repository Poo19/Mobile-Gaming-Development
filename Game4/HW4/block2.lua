...
{text="   Hit Points:0  Life = 8 ",
x=display.contentCenterX-200, y=80,
fontSize=30});
...
function scene:create( event )
   ...
   Background:setFillColor(0.6,0.2,0.3);
   ...
   --- Ball----
    ...
    ball:applyForce(0.5,9,ball.x,ball.y);
    ball: setFillColor (0,1,0)
    sceneGroup:insert(ball);

    ball2 = display.newCircle (display.contentCenterX, display.contentCenterY-30, 20);
    physics.addBody (ball2, "dynamic", {bounce=1.05, radius=20} );
    ball2:applyForce(0.5,9,ball2.x,ball2.y);
    ball2: setFillColor (0,1,1)
...
function scene:show( event )
   ...
   elseif ( phase == "did" ) then
    ...
    if (life==0) then event.target: removeSelf(); return end;
    if (event.phase=="ended") then
        ...
        if (event.other == bottom ) then
            if (ball  ~= nil) then
            event.target:setFillColor(math.random(0,1),math.random(0,0.5),math.random(0,1))
            life = life - 1; 
            if (life==0) then event.target: removeSelf();  end
        end
        end
        show.text =
    ...
    ball:addEventListener("collision", removeProjectile);
     local function removeProjectile2 (event)
      print ("collision Entered2")
       if (life==0) then event.target: removeSelf(); return end;
    if (event.phase=="ended") then
        if (event.other.tag == "Blue") then
            event.other.pp:hit();
        ...
        if (event.other == bottom ) then
            if (ball2  ~= nil) then
            event.target:setFillColor(math.random(0,0.5),math.random(0,1),math.random(0,1))
            life = life - 1; 
            if (life==0) then event.target: removeSelf();  end
        end
        end
    ...
    ball2:addEventListener("collision", removeProjectile2);
